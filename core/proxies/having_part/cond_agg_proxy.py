...
class HavingCondAggNetProxy(ModuleProxy):
    def __init__(self, base_net, predict_mode=False, train_data_holder=None, valid_data_holder=None, test_data_holder=None):
        super(HavingCondAggNetProxy, self).__init__(predict_mode, train_data_holder, valid_data_holder, test_data_holder)
        self._init_env(base_net, CondAggNet, 'Having', 'agg', True)

    def predict(self, top=1, keyword=None, target_path=None, extra=None):
        result = super().predict(top, 'agg', '/Having/agg', extra=self.prefix)
    ...
    def _init_train(self, base_net, target_net, part_name, file_name, tensor=False):
        ...
        with open(DLSet.main_folder_link % 'Train' + '/Having/X_gt_sup_agg', 'r') as f:
            info = json.load(f)
            self.prefix = np.array(info['prefix'], dtype=np.int32)

        with open(DLSet.main_folder_link % 'Validation' + '/Having/X_gt_sup_agg', 'r') as f:
            info = json.load(f)
