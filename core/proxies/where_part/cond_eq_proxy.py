...
class WhereCondEqNetProxy(ModuleProxy):
    ...
    def predict(self, top=1, keyword=None, target_path=None, extra=None):
        result = super().predict(top, 'eq', '/Where/eq', extra=self.prefix)
    ...
    def _init_train(self, base_net, target_net, part_name, file_name, tensor=False):
        ...
        with open(DLSet.main_folder_link % 'Train' + '/Where/X_gt_sup_eq', 'r') as f:
            info = json.load(f)
            self.prefix = np.array(info['prefix'], dtype=np.int32)

        with open(DLSet.main_folder_link % 'Validation' + '/Where/X_gt_sup_eq', 'r') as f:
            info = json.load(f)
