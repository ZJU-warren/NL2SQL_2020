...
class WhereNCondNetProxy(ModuleProxy):
    def __init__(self, base_net, predict_mode=False, train_data_holder=None,
                 valid_data_holder=None, test_data_holder=None):
        super(WhereNCondNetProxy, self).__init__(predict_mode, train_data_holder, valid_data_holder, test_data_holder)
        self._init_env(base_net, NCondNet, 'Where', 'N', True)
    ...
    def predict(self, top=1, keyword=None, target_path=None):
        result = super().predict(top, 'N', '/Where/N')
